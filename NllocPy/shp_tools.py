...
# Create a dictionary to map Python types to OGR types
OGRTypes = { int: ogr.OFTInteger, 
             str: ogr.OFTString, 
             float: ogr.OFTReal,
             'str': ogr.OFTString,
             'str_': ogr.OFTString, 
...
def write_points( x, y, 
    ...
    while True:
        try:             
            ...
            if isinstance(prj_code, int):
                srs.ImportFromEPSG(prj_code)
            else:
            ...
            if fields is not None:
                for f in fields: 
                    if type(fields[f][0]).__name__ == 'str' :
                        print( 'okoko')
                        field_type = 'str_'
                    elif 'U' in type(fields[f][0]).__name__ :
                        field_type = 'str_' 
                    else:
                        field_type = type(fields[f][0]).__name__
                    print( f, field_type )
                    Field = ogr.FieldDefn(f, OGRTypes[field_type]) 
            ...
            for n, (i, j) in enumerate(zip(x,y)):
                ...
                if fields is not None:
                    for fi, f in enumerate(fields):
                       print( f, fields[f][n] )
                       feature.SetField(field_names[fi], fields[f][n])
...
def write_ply(xy_rings, fields=None, name='points_shp', path=None, prj_code=4326, kml=True, csv=False): 
    ...
    while True:
        try:      
            ...
            # Define srs 
            srs = osr.SpatialReference()
            srs.ImportFromProj4(utl.prj_(prj_code))
            # Define layer name and type
            ...
            print(f"Exception occurred: {e} at line {line_number}")
            del ply_data_source  
            return None
...
# -----------------------------------------------------------------------------
def plot_ply( shp_ply, facecolor='0.8', edgecolor='black', lim=None,
              plot=True ):
