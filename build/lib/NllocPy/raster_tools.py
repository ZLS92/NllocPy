...
def xy2rasterVal( raster, x, y, prjcode=4326, band=1, close=False ) :
    ...
    row, col = xy2rasterIdx( raster, x.ravel(), y.ravel(), prjcode=prjcode )
...
# -----------------------------------------------------------------------------
def raster2xyz( raster, nodata=False, condition=None, sep=False, midpx=False, lim=None,
                plot=False, cmap='rainbow', vmin=None, vmax=None, s=None, prjcode=None,
                close=False ) :
    ...
    if ( lim is not None ) or ( prjcode is not None ) :
        raster_new = raster_warp( raster, lim=lim, out_prjcode=prjcode )
        raster = None 
        raster = raster_new
    ...
    if type( nodata ) in ( int, float ) :
        xyz[z==nan,2], z[z==nan] = nodata, nodata
    if condition != None:
        xyz=xyz[condition(z)]
    if sep==True:
...
def raster_warp( raster,
    ...
    # Unlink raster from file system memory if already there
    if raster.GetDescription() != dest_path_name :
        if os.path.exists(dest_path_name):
            try:
                gdal.Unlink(dest_path_name)
    ...
    raster_out=gdal.Warp(dest_path_name, raster,
                         dstSRS = out_prjcode,
                         xRes = xRes,
                         yRes = yRes,
                         outputBounds = lim,
...
def array2raster( array, lonmin=0, latmax=0, rx=1, ry=1, reference_raster=None,
                  ...
                  nodata=9999, new_folder=True, close=True, adjst_lim=True,
